    ...
    async function handleExit() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(payload?.message ?? "Falha ao encerrar impersonação.");
            }

            router.push("/protected/superadmin/tenants");
            router.refresh();
        } catch (err) {
            setError(err instanceof Error ? err.message : "Falha ao encerrar impersonação.");
        } finally {
            setLoading(false);
        }
    }
        ...
        <div className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-amber-950">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-sm font-semibold">
                    ...
                    Você está acessando como administrador da conta {companyName ?? "selecionada"}.
                </div>
                <button
                    type="button"
                    onClick={handleExit}
                ...
                >
                    {loading ? "Saindo..." : "Sair da impersonação"}
                </button>
            </div>

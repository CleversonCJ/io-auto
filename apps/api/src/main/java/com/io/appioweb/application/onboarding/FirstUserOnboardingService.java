...
public class FirstUserOnboardingService {
    ...
    public FirstUserRegisterResponse register(FirstUserRegisterRequest request, String payloadJson) {
        ...
        try {
            ...
            // Find or create company by CNPJ
            ...
            if (!cnpj.isBlank()) {
                ...
                company = companyRepo.findByEmail(reg.companyEmail() != null ? reg.companyEmail().trim().toLowerCase(Locale.ROOT) : email).orElse(null);
            }
            ...
            // Update company fields (never set to ACTIVE here)
            company.setName(notBlankOr(reg.nomeFantasia(), company.getName(), reg.razaoSocial(), "Empresa"));
            if (reg.razaoSocial() != null && !reg.razaoSocial().isBlank()) company.setRazaoSocial(reg.razaoSocial().trim());
            if (reg.nomeFantasia() != null && !reg.nomeFantasia().isBlank()) company.setNomeFantasia(reg.nomeFantasia().trim());
            company.setEmail(reg.companyEmail() != null && !reg.companyEmail().isBlank() ? reg.companyEmail().trim().toLowerCase(Locale.ROOT) : email);
            if (!cnpj.isBlank()) company.setCnpj(cnpj);
            if (!whatsapp.isBlank()) company.setWhatsappNumber(whatsapp);
            if (reg.endereco() != null && !reg.endereco().isBlank()) company.setEndereco(reg.endereco().trim());
            if (reg.cidade() != null && !reg.cidade().isBlank()) company.setCidade(reg.cidade().trim());
            if (!uf.isBlank()) company.setUf(uf);
            if (reg.cep() != null && !reg.cep().isBlank()) company.setCep(reg.cep().trim().replaceAll("\\D", ""));
            company.setUpdatedAt(Instant.now());
            // Force INACTIVE – backend decides status
            if (companyCreated) {
                company.setStatus(CompanyStatus.INACTIVE.name());
            }
            ...
            user = existingUsers.stream()
                    .filter(u -> u.getCompanyId().equals(company.getId()))
                    .findFirst()
                    .orElse(null);

            if (user == null && !existingUsers.isEmpty()) {
                // User exists in a different company – use global lookup
            ...
            if (user == null) {
                ...
                // Create default team (team_id is NOT NULL)
            ...
            // Update user fields (never activate here)
            user.setEmail(email);
            ...
            if (reg.profileImageUrl() != null) user.setProfileImageUrl(reg.profileImageUrl());
            user.setUpdatedAt(Instant.now());
            // Force INACTIVE on register
            if (userCreated) {
                user.setActive(false);
                user.setPrimary(true);
            }
            ...
            BigDecimal valor = comercial != null && comercial.valorPagoCliente() != null
                    ? new BigDecimal(comercial.valorPagoCliente().toString()) : BigDecimal.ZERO;
            String recorrencia = comercial != null && comercial.recorrenciaPagamento() != null
                    ? comercial.recorrenciaPagamento().trim() : "mensal";
            String origem = comercial != null && comercial.origem() != null ? comercial.origem().trim() : "";
            ...
            log.info("[Onboarding:Register] Completed – company={} user={} created={}", company.getId(), user.getId(), companyCreated || userCreated);
    ...
    public FirstUserActivateResponse activate(FirstUserActivateRequest request, String payloadJson) {
        ...
        try {
            ...
            String paymentStatus = request.paymentStatus() != null ? request.paymentStatus().trim().toUpperCase(Locale.ROOT) : "";
            if (!VALID_ACTIVATION_STATUSES.contains(paymentStatus)) {
                ...
                eventRepo.save(event);
                throw new BusinessException("ONBOARDING_INVALID_PAYMENT_STATUS",
            ...
            if (subscriptionId.isBlank() && paymentId.isBlank()) {
                throw new BusinessException("ONBOARDING_MISSING_REFERENCE", "subscriptionId ou paymentId é obrigatório.");
            }
            ...
            if (subscription == null) {
                throw new BusinessException("ONBOARDING_SUBSCRIPTION_NOT_FOUND",
            ...
            JpaCompanyEntity company = companyRepo.findById(companyId)
                    .orElseThrow(() -> new BusinessException("ONBOARDING_COMPANY_NOT_FOUND", "Empresa não encontrada: " + companyId));

            JpaUserEntity primaryUser = findPrimaryUser(companyId);
            if (primaryUser == null) {
                throw new BusinessException("ONBOARDING_USER_NOT_FOUND", "Usuário primário não encontrado para empresa: " + companyId);
            }
            ...
            // Sync Asaas subscription description (best effort – failures don't break activation)
            if (!subscriptionId.isBlank()) {
                try {
                    String planName = request.planName() != null && !request.planName().isBlank()
                            ? request.planName().trim() : subscription.getDescription();
            ...
            log.info("[Onboarding:Activate] Completed – company={} user={} subscription={}", companyId, primaryUser.getId(), subscription.getId());
    ...
    public SendAccessEmailResponse sendAccessEmail(SendAccessEmailRequest request, String payloadJson) {
        ...
        // Idempotency check on email_outbox
        ...
        if (existingEmail.isPresent()) {
            ...
            JpaEmailOutboxEntity e = existingEmail.get();
            return new SendAccessEmailResponse(
                    EmailStatus.SENT.name().equals(e.getStatus()),
                    e.getProviderId() != null ? e.getProviderId() : ""
            );
        }

        // Idempotency check on onboarding_events
        Optional<JpaOnboardingEventEntity> existingEvent = eventRepo.findByIdempotencyKey(key);
        if (existingEvent.isPresent() && OnboardingEventStatus.DONE.name().equals(existingEvent.get().getStatus())) {
        ...
        try {
            ...
            // Validate user and company are ACTIVE
            ...
            if (companyId != null) {
                ...
                if (company == null || !CompanyStatus.ACTIVE.name().equals(company.getStatus())) {
                    throw new BusinessException("ONBOARDING_COMPANY_NOT_ACTIVE",
            ...
            if (userId != null) {
                ...
                if (user == null || !user.isActive()) {
                    throw new BusinessException("ONBOARDING_USER_NOT_ACTIVE",
                            "Usuário não está ativo. Ative primeiro via /activate.");
                }
            }

            // Determine set-password URL
            ...
            String resolvedLoginUrl = request.loginUrl() != null && !request.loginUrl().isBlank()
                    ? request.loginUrl().trim() : this.loginUrl;

            // Create email outbox entry
            JpaEmailOutboxEntity outbox = emailOutboxService.createFirstUserAccessEmail(
                    key, email, nome, resolvedLoginUrl, setPasswordTokenUrl, payloadJson
            );
    ...
    private JpaOnboardingSubscriptionEntity findOrCreateSubscription(String paymentId, String subscriptionId) {
        ...
        sub.setAsaasDescriptionSynced(false);
        return sub;
    }

    private JpaOnboardingSubscriptionEntity findSubscriptionForActivation(String subscriptionId, String paymentId) {
        if (!subscriptionId.isBlank()) {
            Optional<JpaOnboardingSubscriptionEntity> sub = subscriptionRepo.findByAsaasSubscriptionIdForUpdate(subscriptionId);
            if (sub.isPresent()) return sub.get();
        }
        if (!paymentId.isBlank()) {
            Optional<JpaOnboardingSubscriptionEntity> sub = subscriptionRepo.findByAsaasPaymentIdForUpdate(paymentId);
            if (sub.isPresent()) return sub.get();
        }
        return null;
    }
    ...
    private JpaUserEntity findPrimaryUser(UUID companyId) {
        return userRepo.findAllByCompanyId(companyId).stream()
                ...
                .findFirst()
                .orElseGet(() -> userRepo.findAllByCompanyId(companyId).stream().findFirst().orElse(null));
    }

    private String generatePasswordResetToken(UUID userId) {
        // Check if an existing unused token exists
        ...
        token.setExpiresAt(Instant.now().plusSeconds(72 * 3600)); // 72 hours
        token.setUsed(false);
    ...
    private String generateSetPasswordUrl(UUID userId) {
        ...
        if (setPasswordBaseUrl.isBlank()) return "";
    ...
    private FirstUserRegisterResponse buildRegisterResponseFromEvent(JpaOnboardingEventEntity event) {
        // Return a generic idempotent response – the event was already processed
    ...
    private String normalizeEmail(String email) {
        if (email == null || email.isBlank()) throw new BusinessException("ONBOARDING_INVALID_EMAIL", "E-mail é obrigatório.");
        String normalized = email.trim().toLowerCase(Locale.ROOT);
        if (!normalized.contains("@")) throw new BusinessException("ONBOARDING_INVALID_EMAIL", "E-mail inválido: " + normalized);
        return normalized;
    }
    ...
    private LocalDate parseDate(String dateStr) {
        ...
        try {
            return LocalDate.parse(dateStr.trim().substring(0, Math.min(10, dateStr.trim().length())));
        } catch (Exception e) {
            return null;
        }
    }
